...
func splitAndMergeCompactorFactory(ctx context.Context, cfg Config, logger log.Logger, reg prometheus.Registerer) (Compactor, Planner, error) {
	...
	compactor, err := tsdb.NewLeveledCompactorWithChunkSize(ctx, reg, logger, cfg.BlockRanges.ToMilliseconds(), nil, chunks.DefaultChunkSegmentSize, nil, true, sharding.ShardFunc)
	if err != nil {
		return nil, nil, err
	}
