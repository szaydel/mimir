...
* [BUGFIX] Runtime config: send `X-Cluster` validation header when fetching runtime config from a cluster-validated HTTP endpoint. New `-runtime-config.client.cluster-validation.label` flag (inherits from `-common.client-cluster-validation.label`). #15226
